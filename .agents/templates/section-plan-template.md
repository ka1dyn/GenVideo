# {Section} 애니메이션 기획서

## 섹션 개요

- 총 길이: {duration}ms ({frames} frames)
- 원본 대본 문장 수: N
- 시퀀스 수: M

## 자막 구성

- 각 seq별로 자막을 구성할 때, 원본 대본을 기반으로 자막을 구성한다.
- seq에 포함된 자막 길이가 길다면, 최소 타임프레임 단위로 나눠서 타이밍을 분할한다.
- [중요] 이전 seq의 endMs와 다음 seq의 startMs가 다르다면, 이전 seq의 endMs를 다음 seq의 startMs로 설정하고 프레임을 재계산한다. 또한 마지막 seq의 endMs는 섹션의 전체 '총 길이(duration)'와 일치시켜 영상 끝부분의 묵음 여백(Tail)까지 포함시킨다. 만약 seq 사이나 끝에 공백 프레임이 생기면 시각 시퀀스와 오디오/자막 재생 시점이 어긋나게 됩니다.

**반드시 계획에 표기해야 할 사항**
자막 타임라인 구성은 다음과 같으며, 모든 영상, 자막 타이밍은 이 내용을 기반으로 한다.

```
Seq 1: {startMs}ms ~ {endMs}ms ({frames} frames) <--- 새로 계산된 값
    - **타이밍 근거**: 타임스탬프 #X ~ #Y 구간 기반 추정 (startMs ~ endMs)
    - 자막 1:
        원본 텍스트: {추출된 실제 타겟 기준 원본 문장}
        타임스탬프: {startMs}ms ~ {endMs}ms ({frames} frames) <--- 이 부분은 오디오 타임스탬프를 기반으로 계산된 값과 일치해야함
    - 자막 2:
        원본 텍스트: ...
        타임스탬프: ...
    ...
Seq 2: {startMs}ms ~ {endMs}ms ({frames} frames)
    - 자막 1:
        원본 텍스트: ...

    ...
```

seq1의 endMs와 seq2의 startMs의 정합성을 검토하고, frame 수가 잘 맞는지 확인합니다.

## 시퀀스 구성

**핵심 가치**

- 시청자가 지루하지 않도록 영상에 다채로움, 리듬감, 재미를 부여하세요.
- 시퀀스 내에 세부 시퀀스를 추가할 수 있습니다. 4초 이상 지속되는 시퀀스는 구현 단계에서 내부 시퀀스로 분할하는 것을 고려하고 계획하세요

### Seq 1: seq1.tsx ({startMs}ms ~ {endMs}ms, {frames} frames) <-- 상단에서 작성한 구조와 타이밍이 일치해야함.

- **비주얼 컨셉 기획**:
  - 어떤 그래픽을 사용해서 묘사할 지
  - 어떤 텍스트를 배치하고, 어떤 색상, 폰트를 사용할 지
  - 애니메이션은 어떻게 할 지(텍스트 강조, 등장/퇴장 등 창의력을 발휘하여 다채롭게 구성)
  - 디자인 가이드에 따라 4초 이상 지속되는 애니메이션이라면 세부 시퀀스를 어떻게 나눌 지 계획에 포함하세요.
  - 어떻게 화면 요소들을 배치할지(단순 중앙 정렬을 지양하고, 분할/비대칭/3D 등 창의적이고 다채로운 레이아웃 구도 사용하세요)

  지루하지 않도록 **타임라인 구간 전반에 걸쳐** 각 요소와 애니메이션을 배치하는 계획을 상세하게 작성하세요.

- **세부 시퀀스 (Sub-sequences) 분할 계획** (권장):
  - 시퀀스의 길이가 길거나(4초 이상) 한 시퀀스 내에서 장면 전환이 필요한 경우, 내부적으로 세부 구간(SubSeq)을 나누어 기획합니다.
  - **[검증 필수]** 분할된 모든 SubSeq의 프레임 구간(종료 지점)이 현재 Seq{N}에 할당된 전체 `{frames}` 길이를 절대 초과하지 않도록 계산하세요. 마지막 SubSeq의 종료 시점은 가급적 전체 프레임 수와 딱 맞아떨어져야 합니다.
  - SubSeq 1 (0 ~ N frames): [어떤 동작/텍스트/그래픽이 나타나는지 설명]
  - SubSeq 2 (N ~ M frames): [이어서 어떤 변화가 일어나는지 설명]
  - ... (필요한 만큼 세분화)

- **전환 (트렌지션) 계획**: 다음 seq로 넘어갈 때 어떤 효과로 넘어갈 지 계획(마지막 seq는 제외)

### Seq 2: seq2.tsx ({startMs}ms ~ {endMs}ms, {frames} frames)

... (이하 루프)
