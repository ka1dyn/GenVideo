...
- **Phase 5: Implement** → 모델이 기획서를 기반으로 Remotion 컴포넌트 순차 구현
- **Phase 6: Preview** → Remotion Studio에서 프리뷰 확인
...
src/projects/{project_id}/
    theme.ts                        <--- (Phase 5에서 생성 예정) 디자인 시스템의 상수 모음
    {section}/                      <--- {section}_plan.md를 기반으로 Phase 5(Implement) 단계에서 구현할 영역
        {section}_subtitles.ts      <--- (Phase 5에서 생성 예정) 해당 섹션의 자막 타이밍 배열
        {section}.tsx               <--- 해당 섹션의 최상위 Series 래퍼 및 Audio 컴포넌트
        sequences.tsx               <--- (Phase 5에서 생성 예정) 각 section의 시퀀스 컴포넌트 모음
...
**주의**: 이 단계는 AI가 실행하지 않으며 사용자가 직접 별도의 workflows에서 실행합니다.
...
- **파일이 있으면**: 파일을 읽지 말고 우선 이 단계를 건너뜁니다.
...
- `{section}_final_timeline.json` 파일들을 읽습니다.
...
사용자가 기획안 (Phase 4) 을 승인했다면, 본격적인 프로젝트 구현을 시작합니다.
...
- 순차적으로 기획서에 따라 `{section}_subtitles.ts` 및 단일 `sequences.tsx` 파일을 생성합니다.
  (모든 시퀀스(씬)는 복잡도를 낮추기 위해 `sequences.tsx` 파일 내부에 개별 하위 컴포넌트(`Scene1`, `Scene2` 등)로 분할하여 하나로 조립하세요.)
