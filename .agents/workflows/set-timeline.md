...
### 자막 가독성 최적화 (Formatting)

최종 타임라인의 `sentence` 값은 화면에 즉시 노출되는 자막 텍스트입니다. 타이밍(words)은 보존하되, 다음 규칙을 적용하여 `sentence`를 가공하세요.
- **문장 텍스트 정제:** 짝이 맞지 않거나 문장 전체를 감싸는 불필요한 큰따옴표(`"`), 혹은 애매하게 끊기는 끝 쉼표(`,`) 등 어색한 기호는 전부 깔끔하게 제거하세요.
- **지능형 자동 줄바꿈(`\n`):** sentence 길이가 약 22자를 초과할 경우, 중앙에서 가장 가까운 띄어쓰기나 쉼표(`,`)를 기준으로 `\n` 개행 문자를 삽입해 문장을 두 줄로 분할해 주세요.
- **주의:** 텍스트 가공 중에도 내부의 `words` 배열 데이터(타이밍 및 개별 텍스트)는 100% 원본 그대로 보존해야 합니다.

> 💡 **[권장되는 자동화 방식]**
> 직접 수동으로 거대한 JSON 파일을 작성하려 하면 오류가 발생하기 쉽습니다! 이 워크플로우를 완벽히 수행하려면, AI가 직접 수작업으로 JSON을 완성하는 대신 **"Node.js 매핑 스크립트(`generate_timeline.js` 등)"를 작성하여 터미널에서 실행하는 하이브리드 파이프라인 형태**로 작업을 진행하세요.
