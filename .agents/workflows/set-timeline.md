...
> - 만약 원본 대본에 대응되는 타임스탬프가 없다면, 작업을 중지하고 사용자에게 즉각 사용자에게 새 컨텍스트 파일을 요청하세요.

**중요**

### 진행 단계

- `public/{project_id}/{section}/{section}_context.md`의 원본 대본 문장을 기준으로 sentence를 나눕니다.
- 해당 sentence에 포함되는 단어들의 타임스탬프를 확인하여 원본 단어와 비교합니다. 누락된 단어가 없는지 확인하고 사용자에게 어떻게 처리할 것인지 알리세요.
- 각 sentence의 길이(durationInFrames)를 분석합니다. `마지막 단어의 endFrame - 첫번째 단어의 startFrame`로 계산 가능합니다.
- durationInFrames가 절대로 0이 되어선 안됩니다. 0이라는건 단어가 누락되거나 계산이 틀렸다는 뜻이기 때문에 다시한번 확인하고 사용자에게 알립니다.
