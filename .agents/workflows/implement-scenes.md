...
맥락 파악이 끝났다면, `src/projects/{project_id}/{section}/sequences.tsx` 파일을 엽니다. 이 파일 내에 존재하는 각 Scene을 **최대 2개씩 묶어서(Chunk 단위)** 순차적으로 작업하며, 마지막 Scene이 끝날 때까지 다음 1~5번 과정을 반복 수행합니다.

> [!CAUTION]
> **Batch Processing 금지 및 물리적 순서 준수**
>
> 1. 절대로 전체 섹션(5~10개 씬)을 한꺼번에 기획하지 마세요.
> 2. **`Scene N`의 구현 코드가 코드 파일에 저장되기 전까지, `Scene N+2`의 JSDoc 기획(2-2단계)을 시작하는 것은 엄격히 금지됩니다.**
> 3. `implementation_plan.md`를 작성할 때도 오직 **현재 작업 중인 Chunk(2개 씬)**에 대해서만 상세 계획을 수립하세요.
...
작업 대상인 현재 Chunk의 Scene 상단 JSDoc 주석(`/** [Scene X] ... */`) 영역을 자세히 읽고 기획을 작성합니다.

- 주석 내의 **`{FILL_S*_VISUAL: ...}`** 및 **`{FILL_S*_SVG: ...}`** 마커 부분을 **본인이 구상한 구체적인 기획 내용으로 교체**합니다.
- **주의**: 현재 Chunk에 포함되지 않은 다음 순서의 씬들은 보류(Leave as is) 상태로 둡니다.
...
### 2-5. 직후 자체 코드 검토 (Self-Correction) 및 중간 확인

코딩을 마친 직후, 다음 Chunk로 넘어가기 전에 아래 사항을 점검하고 **필요시 즉시 수정**하세요.
...
### 3. 최종 검토 및 완료 보고 (Final Review)

해당 `{section}`의 모든 Scene 작업이 완료되었다면 다음을 진행합니다.
