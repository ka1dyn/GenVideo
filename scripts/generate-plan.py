...
- 🚫 금지 패턴 (이것만 피하면 됩니다):
  ...
  4. 그림은 일반 웹 컴포넌트로 표현이 불가능한 경우에만, 제한적이고 임팩트 있게 사용하세요. 그림이 자주 등장하면 오히려 임팩트가 사라집니다.
...
def generate_plan_for_section(project_id, section):
    ...
    for i, sentence in enumerate(sentences, 1):
        ...
        scene_str += f"- 비주얼 컨셉: {{FILL_S{i}_VISUAL: 이 씬의 레이아웃, 화면에 표시할 텍스트(단어/짧은 구절), 애니메이션 연출을 2~4문장으로 요약.(타임라인 전체에 걸쳐 지루하지 않게 기획) 단어는 고유명사, 약어를 제외하고 전부 한국어로 작성한다.}}\n"
