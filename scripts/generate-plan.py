...
- 깔끔한 레이아웃, 핵심을 전달하는 간결한 텍스트와 다이어그램이 있고, 여백을 넉넉하게 확보하세요
- 데이터의 흐름, UI 컴포넌트의 배치 변화, 타이포그래피, 추상적인 기하학 도형을 활용하여 시각화하세요. 화려한 곡선보다는 깔끔한 직선, 도형 위주의 심플함을 선호합니다.

- 🚫 금지 패턴 (이것만 피하면 됩니다):
  1. 나레이션의 단어를 그대로 그림으로 옮기지 마세요. ("뇌" → 뇌 SVG, "지도" → 세계지도 등 일차원적인 직역 금지) 맥락에 맞게 씬을 간결하고 창의적으로 기획하세요.
  2. 텍스트는 단어 또는 짧은 구절 수준으로 배치하세요 — 문장 전체를 옮기지 말 것
  3. 하나의 씬에 SVG 일러스트를 2개 이상 배치하지 마세요.
  4. 그림은 일반 웹 컴포넌트로 표현이 부족한 경우에, 임팩트 있게 사용하세요. 각 섹션 당 최소 2개에서 최대 8개 정도의 그림을 사용하세요.
...
- Scene의 길이가 8초 이상으로 길다면, 시청자가 해당 장면에서 지루하지 않도록 In-scene 애니메이션을 구체적으로 기획하여 비주얼 컨셉에 포함시킵니다.
...
def generate_plan_for_section(project_id, section):
    ...
    for i, sentence in enumerate(sentences, 1):
        ...
        scene_str += f"- 비주얼 컨셉: {{FILL_S{i}_VISUAL: 이 씬의 레이아웃, 화면에 표시할 텍스트(단어/짧은 구절), 애니메이션 연출을 2~4문장으로 요약.(타임라인 전체에 걸쳐 지루하지 않게 기획) 단어는 고유명사, 약어를 제외하고 전부 한국어로 작성한다. 임팩트 있는 장면은 Canvas나 3D canvas를 사용하라고 명시한다.(제한적으로 사용)}}\n"
        scene_str += f"- 필요한 그림(svg) 컴포넌트: {{FILL_S{i}_COMPONENT: 텍스트와 UI 레이아웃만으로 충분하면 '없음'으로 작성. 필요한 경우에만 최소한의 SVG 컴포넌트를 기재.}}\n"
