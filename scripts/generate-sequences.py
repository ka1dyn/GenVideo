...
def generate_sequences_for_section(project_id, section, force=False):
    ...
    for i, sentence in enumerate(sentences, 1):
        ...
        tsx_lines.append("  {/* 상단 주석의 원본 텍스트와 비주얼 컨셉을 우선 확인하세요. */}")
        tsx_lines.append("  {/* 실제 구현을 어떻게 할 지 기존 기획에서 크게 벗어나지 않도록 디테일을 수정하여 [비주얼 컨셉] 주석을 수정하세요. 친근하고 스케치 느낌을 살리되, 너무 화려하지 않도록 주의하세요. */}")
        tsx_lines.append("  {/* 간단한 그림은 svg로 따로 작성하며, 복잡한 시각적 요소가 필요하다면 판단 하에 제한적으로 canvas를 사용하세요 */}")
        ...
        tsx_lines.append("      {/* 스케치 느낌을 내기 위한 shared-components의 Wobble, DrawLine을 적절히 활용하세요. 이미 import되어 있습니다. */}")
        tsx_lines.append("    </AbsoluteFill>")
        tsx_lines.append("  );")
        tsx_lines.append("};")
        tsx_lines.append("")
